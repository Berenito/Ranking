...
def get_correct_number_of_sheets(workBook, num_wanted):
    """
    Initialize new WorkBook with given number of Sheets.
    """
    sh_cnt = workBook.Sheets.Count
    if sh_cnt < num_wanted:
        [workBook.Sheets.Add() for i in range(num_wanted - sh_cnt)] 
    elif sh_cnt > num_wanted:
        [workBook.Sheets.Item(1).Delete() for i in range(sh_cnt - num_wanted)] 
...
def get_number_format(col_name):
    ...
    """
    if col_name.startswith('Rating'):
        nf = '0.0'
    elif col_name in ['W_Ratio', 'Opponent_W_Ratio', 'Avg_Point_Diff']:
        nf = '0.000'
    else:
        nf = None
...
def print_df(df, xl_app, sheet, top_left=(1, 1)):
    ...
    sheet.Range(get_excel_range((r + 1, c + 1), (r + n_rows, c + n_cols))).Value = [x[1].tolist() for x in df.fillna('').iterrows()]
    # number format
    for i, col_name in enumerate(df.columns):
        if get_number_format(col_name) is not None:
            sheet.Range(get_excel_range((r + 1, c + 1 + i), (r + n_rows, c + 1 + i))).NumberFormat = get_number_format(col_name)
    # filters
    sheet.Select()
    ...
    sheet.Range(get_excel_range((2, 1), (n_rows, c + n_cols))).FormatConditions.Add(win32com.client.constants.xlExpression, '',
                                                                                '=IF(MOD(SUBTOTAL(103,$A$2:$A2),2)=0,"TRUE","FALSE")')
    sheet.Range(get_excel_range((2, 1), (n_rows, c + n_cols))).FormatConditions(1).Interior.Color =  to_excel_color((243, 243, 243))
    # freeze panes
