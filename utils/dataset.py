...
def process_games(df_games: pd.DataFrame, remove_draws: bool = False) -> pd.DataFrame:
    ...
    if idx_bad_w_l.any():
        ...
        df_games.loc[
            idx_bad_w_l, ["Team_1", "Team_2", "Score_1", "Score_2"]
    ...
    for _, rw in df_games.loc[idx_forfeit].iterrows():
        logger.info(
            f"Forfeit game removed: {rw['Team_1']} vs {rw['Team_2']} at {rw['Tournament']} on {rw['Date']}."
        )
    ...
    if idx_dates_dot_format.any():
        logger.info(
            f"{idx_dates_dot_format.sum()} dates converted from DD.MM.YYYY to YYYY-MM-DD."
        )
        df_games.loc[idx_dates_dot_format, "Date"] = df_games.loc[
            idx_dates_dot_format, "Date"
    ...
    if idx_dates_short_format.any():
        logger.info(
            f"{idx_dates_short_format.sum()} dates converted from YYYY-M-D to YYYY-MM-DD."
        )
        df_games.loc[idx_dates_short_format, "Date"] = df_games.loc[
            idx_dates_short_format, "Date"
        ].apply(lambda x: datetime.strptime(x, "%Y-%m-%d").strftime("%Y-%m-%d"))
    return df_games.sort_values(
        by=["Date", "Tournament", "Team_1", "Team_2"]
...
def get_points_won_matrix(df_games: pd.DataFrame) -> pd.DataFrame:
    """
    Calculate the Points Won Matrix from the Games Table.

    :param df_games: Games Table
    :return: Points Won Matrix
    """
    return (
        duplicate_games(df_games)[["Team_1", "Team_2", "Score_1"]]
...
def get_teams_in_games(
    df_games: pd.DataFrame, date: t.Optional[str] = None
) -> pd.Series:
    """
...
def get_opponents_for_team(
    df_games: pd.DataFrame, team: str, date: t.Optional[str] = None
) -> pd.Series:
    """
    ...
    return (
        (df_games_dupl.loc[df_games_dupl["Team_1"] == team, "Team_2"].unique())
        .rename("Team")
        .sort_values()
    )
...
def get_games_for_teams(
    ...
    teams: t.Union[str, list, pd.Series],
    how: str = "any",
    ...
    if isinstance(teams, str):
        teams = [teams]
    if how == "only":
        df_for_teams = df_games.loc[
            df_games["Team_1"].isin(teams) & df_games["Team_2"].isin(teams)
        ]
    elif how == "any":
        df_for_teams = df_games.loc[
            df_games["Team_1"].isin(teams) | df_games["Team_2"].isin(teams)
        ...
        teams_common = functools.reduce(
            lambda x, y: list(
                set(get_opponents_for_team(df_games, x))
                & set(get_opponents_for_team(df_games, y))
            ),
            teams,
        )
        df_for_teams = df_games.loc[
            df_games["Team_1"].isin(teams_common)
            | df_games["Team_2"].isin(teams_common)
        ]
    return df_for_teams


def get_summary_of_games(
    df_games: pd.DataFrame, date: t.Optional[str] = None
) -> pd.DataFrame:
    """
...
def get_summary_of_tournaments(
    df_games: pd.DataFrame, date: t.Optional[str] = None
) -> pd.DataFrame:
    """
...
def get_calendar_summary(df_games: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(date_range) == 0:
        date_range = [
            (pd.to_datetime(date_last) + pd.tseries.offsets.Week(weekday=6)).strftime(
                "%Y-%m-%d"
            )
        ]
    ...
    df_calendar["Date_Start"] = (
        pd.to_datetime(df_calendar["Date_End"]) - pd.tseries.offsets.Day(6)
    ).dt.strftime("%Y-%m-%d")
    ...
    df_calendar["N_Tournaments"] = df_calendar.apply(
        lambda x: df_games.loc[
            df_games["Date"].between(x["Date_Start"], x["Date_End"])
        ]["Tournament"].nunique(),
        axis=1,
    )
...
def get_games_graph(df_games: pd.DataFrame, date: t.Optional[str] = None) -> nx.Graph:
    ...
    if date is not None:
        df_games = df_games.loc[df_games["Date"] <= date]
    df_connected_init = pd.DataFrame(
        0, index=teams, columns=teams
    )  # To ensure that all the teams are in the DataFrame
    df_connected = (
        duplicate_games(df_games)
        .groupby(["Team_1", "Team_2"])["Tournament"]
        .count()
        .rename("N_Games")
    )
    df_connected = (
        df_connected.reset_index()
        .pivot(index="Team_1", columns="Team_2", values="N_Games")
        .fillna(0)
    )
    df_connected = (
        df_connected_init.add(df_connected, fill_value=0).astype("int").clip(upper=1)
    )
    return nx.from_pandas_adjacency(df_connected)


def get_graph_components(
    graph_connections: nx.Graph, teams: t.Union[list, pd.Series]
) -> pd.Series:
    """
    ...
    components_raw = sorted(
        components_raw, key=len, reverse=True
    )  # Sort list of lists by length in descending order
...
def get_shortest_paths(g: nx.Graph, teams: t.List[str]) -> pd.Series:
    ...
    df_shortest_path_len = (
        pd.concat(
            [
                df_shortest_path_raw[c].apply(
                    lambda x: len(x) if isinstance(x, list) else x
                )
                for c in df_shortest_path_raw.columns
            ],
            axis=1,
        )
        - 1
...
def get_ranking_metrics(
    df_games: pd.DataFrame, algo_name: str = ""
) -> t.Tuple[float, float]:
    """
    ...
    df_games["Resid_1"] = (
        df_games[f"Game_Rank_Diff{algo_name}"] - df_games[f"Team_Rank_Diff{algo_name}"]
    )
    df_games["Resid_2"] = -df_games["Resid_1"]
    df_games.loc[df_games[f"Is_Ignored{algo_name}"] == 1, f"Game_Wght{algo_name}"] = 0
    rmse = np.sqrt(
        safe_weighted_avg(df_games["Resid_1"] ** 2, df_games[f"Game_Wght{algo_name}"])
    )
    ...
    sum_resid_teams = df_games_extended.groupby("Team").apply(
        lambda x: safe_weighted_avg(
            x["Resid"], x[f"Game_Wght{algo_name}"], return_sum=True
        )
    )
...
def safe_weighted_avg(
    vals: np.ndarray, wghts: np.ndarray, return_sum: bool = False
) -> float:
    """
